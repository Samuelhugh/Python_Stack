...
# 4 This should return 5 to be printed to the Terminal and should exited the Function and ignore all the other Instructions
...
# 5 This should just print 5 inside the function and then return nothing to (x), but return back to the line it was called from, and print should print the entire outline of the function number_of_great_lakes (Prints none to the Terminal)
...
# 10 I have the print() built-in function calling the function. The Function call is passing two arguments to the function parameters or inputs. So this function should return 8 to the function call which is ALWAYS set equal to what the return statement is (What is Return). And then the interpreter ignores the rest of the instructions.
def addition(b,c):
    return b+c
    return 10
print(addition(3,5))
...
# 14 1 gets printed to the Terminal for the Outside function, 3 gets printed to the Terminal for the bar() Inside function that was called inside of the foo() function, Causing the foo function to pause so the interpreter can run the bar() functions instructions. Nothing gets/is returned from the bar() function. So foo now continues and prints 2 to the Terminal.
def foo():
    print(1)
    bar()
    print(2)
